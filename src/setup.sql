-- Remove existing tables so the script can be run multiple times without errors
DROP TABLE IF EXISTS project CASCADE;
DROP TABLE IF EXISTS organization CASCADE;
DROP TABLE IF EXISTS category CASCADE;

-- Create the Category table (only if your project uses categories)
CREATE TABLE category (
    category_id SERIAL PRIMARY KEY,
    name VARCHAR(255) NOT NULL
);

-- Create the Organization table, including contact details and an optional image
...
-- Create the Project table and link each project to an organization
...
-- Add sample projects for Organization 1 (Bright Futures Youth)
...
-- Add sample projects for Organization 2 (Green Earth Collective)
INSERT INTO project (organization_id, title, description, location, date) VALUES
...
-- Add sample projects for Organization 3 (Wellness Together)
INSERT INTO project (organization_id, title, description, location, date) VALUES
...
-- Add sample projects for Organization 4 (Neighborhood Care)
INSERT INTO project (organization_id, title, description, location, date) VALUES
